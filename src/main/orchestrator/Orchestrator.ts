...
export class Orchestrator {
  ...
  private buildAgentPrompt(command: string, context: DesktopContext, resolution: string): string {
    ...
    parts.push(`Mkdir:     powershell.exe -NoProfile -Command '$OutputEncoding=[Console]::InputEncoding=[Console]::OutputEncoding=New-Object System.Text.UTF8Encoding; New-Item -Path "新文件夹" -ItemType Directory'`)
    parts.push(`Move:      powershell.exe -NoProfile -Command '$OutputEncoding=[Console]::InputEncoding=[Console]::OutputEncoding=New-Object System.Text.UTF8Encoding; Move-Item -LiteralPath "文件.txt" -Destination "目标文件夹\\"'`)
    parts.push(`Rename:    powershell.exe -NoProfile -Command '$OutputEncoding=[Console]::InputEncoding=[Console]::OutputEncoding=New-Object System.Text.UTF8Encoding; Rename-Item -LiteralPath "文件.txt" -NewName "新名.txt"'`)
    parts.push(`Delete:    powershell.exe -NoProfile -Command '$OutputEncoding=[Console]::InputEncoding=[Console]::OutputEncoding=New-Object System.Text.UTF8Encoding; Remove-Item -LiteralPath "文件.txt" -Force'`)
    parts.push(``)
